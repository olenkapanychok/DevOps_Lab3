...
priv_stmt='CREATE USER "mydb_slave_user_olenka"@"%" IDENTIFIED BY "mydb_slave_pwd_olenka"; GRANT REPLICATION SLAVE ON *.* TO "mydb_slave_user_olenka"@"%"; FLUSH PRIVILEGES;'
...
start_slave_stmt="CHANGE MASTER TO MASTER_HOST='mysql_master',MASTER_USER='mydb_slave_user_olenka',MASTER_PASSWORD='mydb_slave_pwd_olenka',MASTER_LOG_FILE='$CURRENT_LOG',MASTER_LOG_POS=$CURRENT_POS; START SLAVE;"
